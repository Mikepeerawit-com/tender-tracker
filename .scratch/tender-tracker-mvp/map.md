...
- [WeCom console: registered the app and robot, and observed what WeCom allows](issues/06-register-wecom-app-and-robot.md) — the HITL ticket, and it settled four things by observation. **Group robot webhooks work**; **`mentioned_mobile_list` works** but *only* in **E.164 with a leading `+`** — every other format is silently accepted (`errcode 0`) and never binds, so `users.mobile` needs write-time validation and `errcode 0` is never proof of delivery. **企业可信IP is enforced**: `gettoken` is exempt and misleadingly returns 0, but every business API returns **60020** — so **Vercel serverless cannot host WeCom-facing code** (~$5/mo fixed-IP VM, not Vercel Static IPs at $100/mo), while the robot webhook is exempt. **可信域名 is rejected on ICP filing-entity grounds** — the console demands a domain "whose filing entity is the same as or related to the current company entity" (备案主体), which a Thai entity cannot obtain. **This hands 07 its answer.** Findings: [`research/06-wecom-console.md`](research/06-wecom-console.md). *Still open, blocking nothing:* second-person mention, the 20/min rate limit, a valid non-member-number test.
